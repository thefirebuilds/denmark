...
const { BANKING_INGESTION_START_DATE, isWithinBankingIngestionWindow } = require("../services/banking/bankingIngestionPolicy");
...
(async()=>{
  ...
  const eligible=transactions.filter(tx=>isWithinBankingIngestionWindow(tx.authorized_date||tx.date));
  console.log(JSON.stringify({item_id:exchanged.item_id,ingestion_start_date:BANKING_INGESTION_START_DATE,count:eligible.length,transactions:eligible.slice(0,5).map(tx=>({date:tx.date,name:tx.merchant_name||tx.name,amount:tx.amount,pending:tx.pending,account_id:tx.account_id}))},null,2));
